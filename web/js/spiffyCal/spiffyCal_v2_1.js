...
var scImgPath = sitepath+'js/spiffyCal/images/';

//var scImgPath="../include/spiffyCal/images/";
...
img_Del.src= scImgPath +"btn.gif";

img_Close.src= scImgPath +"btn.gif";
...
function ctlSpiffyCalendarBox(strVarName, strFormName, strTextBoxName, strBtnName, strDefaultValue, intBtnMode) {
	...
	//on 13092011 dilip made all the date text boxes as readonly
	...
	switch (intBtnMode) {
		...
		case 0 :
			...
			strBtnImg=strDefBtnImgPath+'btn.gif';

			img_DateBtn_UP.src=strDefBtnImgPath+'btn.gif';

			img_DateBtn_OVER.src=strDefBtnImgPath+'btn.gif';

			img_DateBtn_DOWN.src=strDefBtnImgPath+'btn.gif';

			img_DateBtn_DISABLED.src=strDefBtnImgPath+'btn.gif';
...
// put these next 2 lines in when the tiny cal btns seem to randomly disappear
			...
			if (this.focusClick==true && this.hideButton==true) {
				...
				if ( scIE ) {
					...
					if (this.hideCombos) {toggleCombos('hidden');}//toggleCombos('hidden');
			...
			//'dipti' if (this.hideCombos) {toggleCombos('hidden');}
	...
	function hide() {
		...
		// 'dipti'if (this.hideCombos) {toggleCombos('visible');}
